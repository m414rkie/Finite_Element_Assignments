...
def graph(out,title,ss,r_vals,i_vals):
    ...
    plt.plot(ss,r_vals)
    plt.plot(ss,i_vals,'r')
    plt.legend(["Stability Results"], loc='upper right')
    plt.xlim((x_l,x_h))
    plt.ylim((y_l,y_h))
    plt.ylabel('Im')
    plt.xlabel('Re')
...
for val in a_vals:
    for lmda in lmda_vals:
        ...
        for l, g in enumerate(results):
            alpha[l] = -speeds[l]*1j*np.log(g/np.absolute(g))

        graph(output,title,speeds,alpha.real,alpha.imag)
