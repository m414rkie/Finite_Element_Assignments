...
def twostp_mat(N,a,lmda):
    ...
    while i < n:
        if i == 0:
            ...
            ll = 0.5*lmda*lmda*((a[i-1]*a[i-1]+a[i]*a[i]+a[i+1]*a[i+1])/3)
            A[i,i-1] = -ll
            A[i,i]   = 1 + 2*ll
            A[i,i+1] = -ll
            B[i,i-1] = ll
...
def graph(out,title,x1_vals,y_vals,y2_vals,y3_vals):
    ...
    plt.legend(["Numerical","Exact","a*lmda"], loc='upper right')
    plt.xlim((x_l,x_h))
    plt.ylim((y_l,y_h))
...
for h in h_vals:
    ...
    while t_c <= t_f + k:
        ...
        graph(output,title,x_vals,u_ctcs_n,u_exact,lmda*sd)
        # 2 step Part
        ...
        graph(output,title,x_vals,u_2s_n,u_exact,lmda*sd)
