...
# This version implements a random wavespeed field.
...
def u_ex(x,t):
    ...
    for i in range(100):
        l_md = 2*i + 1
        sum += ((-1)**i)*(math.cos(Pi*l_md*x)/(Pi*(2*i+1)))*math.exp( \
                                                              -((Pi*l_md)**2)*t)
...
def acc_ord(v):
    ...
    N = 1
    while N < len(v) - 1:
        ord_c = np.log((v[N-1] - v[N])/(v[N]-v[N+1]))/np.log(2)
        sum += ord_c
        print(ord_c)
        N += 1
...
def graph(out,title,x1_vals,y_vals,y2_vals,y3_vals):
    ...
    # y3_vals- holds values of wavespeed adjustment
    ...
    plt.plot(x_vals,y3_vals,'b',alpha=0.3)
    plt.legend(["Numerical","Exact","a*Mu"], loc='upper right')
    plt.xlim((x_l,x_h))
    plt.ylim((y_l,y_h))
...
for h in h_vals:
    ...
    while t_c <= t_f + k:
        ...
        i = 1
        while i < N:
            aa = (sd[i-1]+sd[i]+sd[i+1])/3
            u_ftfs_n[i] = FTCS(u_ftfs_p[i],u_ftfs_p[i-1],u_ftfs_p[i+1],aa,mu)
        ...
        graph(output,title,x_vals,u_ftfs_n,u_exact,mu*sd)
        # CN Part
        ...
        graph(output,title,x_vals,u_cn_n,u_exact,mu*sd)
    ...
    ftfs_l2_norms[q-1] = L2_norm(u_ftfs_p,h)
    cn_l2_norms[q-1]   = L2_norm(u_cn_p,h)
